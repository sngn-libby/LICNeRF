...
from src.compressproj.elic.utils.func import update_registered_buffers, get_scale_table
from src.compressproj.elic.utils.ckbd import *
from src.compressproj.elic.modules.transform import *

class ELIC(CompressionModel):
    def __init__(self, N=192, M=320, n_slice=10, quant="ste", **kwargs):
    ...
    def compress(self, x):
        ...
        tar_cost_time = time.process_time() - start_log_time

        end_log_time_part = 0
        for idx, y_slice in enumerate(y_slices):
            start_log_time_slice = time.process_time()
            slice_anchor, slice_nonanchor = ckbd_split(y_slice)
            if idx == 0:
                # Anchor
                params_anchor = self.entropy_parameters_anchor[idx](hyper_params)
                ...
                params_nonanchor = self.entropy_parameters_nonanchor[idx](torch.cat([local_ctx, hyper_params], dim=1))
                scales_nonanchor, means_nonanchor = params_nonanchor.chunk(2, 1)
                end_log_time_slice = time.process_time()
            ...
            end_log_time_part += end_log_time_slice - start_log_time_slice

        tar_cost_time += end_log_time_part

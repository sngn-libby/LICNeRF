...
def main(argv):
    ...
    train_dataset = ImageFolderDE(args.dataset, split="train", transform=train_transforms, patch_size=args.patch_size)
...
def train_one_epoch(
    ...
    for i, d in enumerate(train_dataloader):
	    ...
	    # Augmentation (PE)
            ...
            pe_depth = torch.cat(pe_depth, dim=1) # n_channel = n_freq * 2 + 1
        d_cat = torch.cat([d[:, :-1, :, :], pe_depth], dim=1) # n_channel - 1
